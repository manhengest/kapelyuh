...
export function useGameSelectors() {
  ...
  return useMemo(
    () => ({
      ...
      matchStats: {
        ...selectMatchStats(state.turnHistory, state.teams, wordTexts),
        matchDurationMs: selectMatchDurationMs(state.turnHistory, state.createdAt),
      },
...
export const STATUS_ROUTE: Partial<Record<GameStatus, string>> = {
  ...
  building_hat: '/game/building-hat',
