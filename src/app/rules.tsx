      ...
      >
        {steps.map((entry) => (
          <View key={entry.title} style={{ width }} className="flex-1 px-6 pt-8">
            <Text className="mb-4 text-2xl font-bold text-slate-900 dark:text-white">{entry.title}</Text>
            <Text className="mb-6 text-base leading-6 text-slate-700 dark:text-slate-300">{entry.body}</Text>
            <View className="rounded-2xl bg-pink-100 px-4 py-4 dark:bg-pink-900/30">
              <Text className="mb-1 text-xs font-bold uppercase text-pink-700 dark:text-pink-300">
                {entry.tipLabel}
              </Text>
              <Text className="text-base text-slate-800 dark:text-slate-200">{entry.tip}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
