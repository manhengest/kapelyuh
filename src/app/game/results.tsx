  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-slate-900">
      ...
      <ScrollView className="flex-1 px-5" contentContainerClassName="py-8">
        <Text className="mb-1 text-center text-2xl font-bold text-slate-900 dark:text-white">
          {strings.results.matchComplete}
        </Text>
        <Text className="mb-6 text-center text-lg text-slate-600 dark:text-slate-300">
          {strings.results.congrats}
        </Text>

        {winners.map((winner) => (
          <WinnerMedal key={winner.id}>
            <View className="mb-6 items-center rounded-3xl bg-yellow-100 px-4 py-6 dark:bg-yellow-900/30">
              <Text className="text-4xl">🏅</Text>
              <Text className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{winner.name}</Text>
        ...
        <View className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">
          <View className="flex-row bg-slate-100 px-3 py-2 dark:bg-slate-800">
            <Text className="flex-1 text-xs font-bold text-slate-700 dark:text-slate-200">Команда</Text>
            <Text className="w-10 text-center text-xs font-bold text-slate-700 dark:text-slate-200">I</Text>
            <Text className="w-10 text-center text-xs font-bold text-slate-700 dark:text-slate-200">II</Text>
            <Text className="w-10 text-center text-xs font-bold text-slate-700 dark:text-slate-200">III</Text>
            <Text className="w-10 text-center text-xs font-bold text-slate-700 dark:text-slate-200">🏅</Text>
          </View>
          {scoreboard.map((row) => (
            <View key={row.teamId} className="flex-row border-t border-slate-200 px-3 py-2 dark:border-slate-700">
              <Text className="flex-1 text-sm text-slate-900 dark:text-white">{row.name}</Text>
              <Text className="w-10 text-center text-sm text-slate-900 dark:text-white">{row.scores.elias}</Text>
              <Text className="w-10 text-center text-sm text-slate-900 dark:text-white">{row.scores.crocodile}</Text>
              <Text className="w-10 text-center text-sm text-slate-900 dark:text-white">{row.scores.association}</Text>
              <Text className="w-10 text-center text-sm font-semibold text-slate-900 dark:text-white">{row.total}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
